...
const ProductTableRow = ({ product }: Props) => {
  ...
  const [imageSectionWidth, setImageSectionWidth] = useState<
    string | undefined
  >();
  ...
  const getContainerSize = useCallback(() => {
    ...
    if (productCompatibilitiesLength > 0 && bbContainer) {
      ...
      const numberOfVisibleBBImageinContainer =
        ...
        Math.floor((bbImageContainerWidth || 0) / bbImageWidth);

      setNumberOfHidenBBImages(Math.max(0, numberOfVisibleBBImageinContainer));
      setImageSectionWidth(
        `${bbImageContainerWidth - (bbImageContainerWidth % bbImageWidth)}px`
      );
    }
  ...
  return (
    <>
      <div
        className={classNames('product-table-row', {
          'product-table-row-opened': isSubTableOpen,
        })}
        data-testid={`product-table-row-${product._id.testApp}`}
        onClick={handleShowSubTable}
      >
        <div className="details-arrow">
          {isSubTableOpen ? <RiArrowUpSLine /> : <RiArrowDownSLine />}
        </div>
        <div>
        ...
        <div className="table-bb-section">
          <div ref={bbContentContainer}>
            <div
              className="table-bb-image"
              style={{ width: imageSectionWidth ?? '75%' }}
            >
            ...
            <div>
              {numberOfHidenBBImages ? (
                <div
                  ...
                  data-tooltip-id="text-tooltip"
                  data-tooltip-content={listOfHiddenBBImages
                    ?.map((bb) => format(bb))
                    .join(', ')}
                >
        ...
        <div data-testid="product-last-update">
          <p className="product-last-update">
            {product.lastUpdate
              ? new Date(product.lastUpdate).toLocaleDateString(locale)
              : ''}
          </p>
        </div>
